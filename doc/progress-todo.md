...
2026-05-11 Deploy to server:
- Commit `37a907b` pushed to `origin/main`. Server pull + restart needed:
  ```bash
  cd /root/hyywaimaoappnew
  git pull origin main
  cd backend
  source venv/bin/activate
  pip install -r requirements.txt  # if new deps
  pm2 restart waimao-backend
  ```
- No new Python dependencies, no new DB migrations. Frontend will auto-deploy via Vercel on push.
