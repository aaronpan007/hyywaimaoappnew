...
### 2026-05-10 开发信上传 Excel 与语言约束修复

测试发现/确认：

- 用户上传 Excel 生成开发信已可用。
- 如果上传表格只有公司名、网站、国家、行业、联系人、邮箱等基础字段，没有系统标准格式里的 `AI分析摘要`、`业务匹配点`、`开发建议`，email-craft 会基于用户提供的基础客户字段 + 当前公司画像写开发信；不会凭空拥有深度客户分析。
- 单独选一个公司重写开发信时，发现生成了中文；开发信语言和写作规范必须严格遵守 `waimao_toolkit_new/skills/email-craft/SKILL.md`。

已修复：

- `backend/app/services/email_craft_pipeline_service.py`：强化 `build_system_prompt()`，英文模式明确要求 `email_subject` 和 `email_body` 全英文，不允许中文句子或中英混写；中文模式明确主题和正文都为中文商务表达。
- `build_user_prompt()`：当客户缺少 AI分析摘要/业务匹配点/开发建议时，明确要求只用已有客户字段和我司信息保守撰写，不编造客户项目、近期动态、规模、采购需求或联系人细节。
- `_generate_one_sync()`：新增语言校验。`language=en` 时如果模型返回中文，会自动重试；三次仍不符合语言要求则该条生成失败，不保存错误语言草稿。

后续验证：

1. 上传只有公司名/邮箱/国家/行业的 Excel，生成英文开发信，确认邮件更保守但仍结合我司画像。
2. 在客户列表单独选一个公司重写开发信，语言选择 EN 时必须生成英文；选择中文时生成中文。
3. 如前端入口仍出现语言选择状态混淆，再给详情抽屉里的“重写开发信”旁边补一个明显的 EN/中文切换。

### 2026-05-10 上传客户表格保留非标准有用列

需求补充：

- 用户上传的 Excel/CSV/Word 表格不一定只有公司名、网站、国家、行业、联系人等标准列。
- 表格可能包含“客户痛点”“项目类型”“采购备注”“主营产品”“需求说明”“跟进记录”等有价值字段，但列名不叫 `AI分析摘要` 或 `业务匹配点`。

已修复：

- `backend/app/utils/file_parser.py`：标准字段照常映射；无法映射到标准字段、但单元格有内容的列会合并为 `user_note`，格式为“用户上传补充信息：列名: 内容”。
- `backend/app/services/lead_service.py`：上传导入线索时保存 `user_note`。
- `backend/app/services/email_craft_pipeline_service.py`：开发信 prompt 会读取 `user_note` 并放入“用户上传/手动补充信息”，用于个性化写信。
- 当没有 AI分析摘要/业务匹配点/开发建议，但有 `user_note` 时，不再视为完全缺少补充信息；AI 可以基于这些用户上传字段写更贴近的开发信。

后续验证：

1. 上传一个包含非标准列（如“客户需求”“项目备注”“产品兴趣点”）的 Excel。
...
```
### 2026-05-10 上传客户表格字段对齐增强

问题判断：

- 上传 Excel/CSV/Word 表格时，现有解析先做静态列名映射，只有当静态映射覆盖率低于 50% 时才调用 LLM 补映射。
- 因此如果表格大部分列能识别，但少数关键列名如“国家/地区”“AI 分析摘要”“业务匹配点”没有命中静态映射，这些列会被归入 `user_note`，导致客户名单里的结构化字段为空。

已修复：

- `backend/app/utils/file_parser.py`：补齐系统标准表头与常见别名映射，包括“国家/地区”“AI 分析摘要”“业务匹配点”“开发信建议/开发切入点”等。
- LLM 列名映射不再只在覆盖率低于 50% 时触发；只要存在未识别且有样例内容的列，就允许 LLM 作为补充映射，但不会覆盖静态映射结果。
- 新增导入后结构化补全：如果字段进入了 `user_note`，且 `country` / `ai_summary` / `business_match` / `outreach_suggestion` 仍为空，会按 20 行一批调用模型，只从已有字段和 notes 里提取信息填回结构化字段；没有依据的字段继续留空，避免编造。

验证：

- `python -m compileall backend/app` 通过。
- 用模拟系统表头 Excel 验证，“国家/地区”“AI 分析摘要”“业务匹配点”“开发建议”可直接映射到结构化字段。

### 2026-05-10 客户名单页直接导入客户表格

体验优化：

- `backend/app/routers/leads.py`：新增 `POST /api/leads/import`，复用现有 `create_leads_from_files()` 解析 Excel/CSV/Word 表格并保存到客户名单。
- `frontend/lib/api.ts`：新增 `importLeadsFromFiles()`。
- `frontend/components/customer-list.tsx`：客户名单右上角新增“导入 Excel 客户信息”按钮，支持选择 `.xlsx/.xls/.csv/.docx`，导入成功后刷新客户列表并提示导入数量；“导出 Excel”保留在旁边。
- `backend/app/services/lead_service.py`：导入来源客户只要 `match_score` 为 0，API 展示值统一返回 `null`，前端显示 `-`；即使 notes 被结构化补全，也不会再显示误导性的 `0.0`。

验证：

- `python -m compileall backend/app` 通过。
- `npm.cmd run build` 通过，包含前端类型检查。

### 2026-05-10 聊天记录菜单交互与客户导入解析再修正

体验/解析问题修复：

- `frontend/components/sidebar.tsx`：聊天记录三点菜单支持点击任意外部区域关闭。
- 重命名不再使用浏览器 `prompt` 弹窗，改为左侧列表内联编辑；按 Enter 或失焦保存，Esc 取消。
- 删除不再弹浏览器确认框，点击“删除”后直接删除该聊天记录。
- `backend/app/utils/file_parser.py`：增强表头归一化，支持空格、斜杠、下划线、括号等差异，例如“国家 / 地区”“业务 匹配 点”“AI分析摘要”都能命中标准字段。
- 行映射时避免空值覆盖同一字段已有非空值，减少标准格式 Excel 出现部分字段漏保存的问题。
- 开发建议字段如果是纯 URL/域名，会被视为无效建议并清空；后续 LLM 结构化补全会尝试根据同一行摘要、业务匹配、行业、notes 等重新生成可执行的开发建议，仍不允许纯链接作为建议。

验证：

- `python -m compileall backend/app` 通过。
- `npm.cmd run build` 通过。
- 用模拟标准格式 Excel 验证：`国家/地区`、`AI分析摘要`、`业务匹配点`、`开发建议` 均可入结构化字段。

线上验证结果：

- 最新代码已部署到服务器，`waimao-api` 与 `waimao-auth` 均为 PM2 online。
- 服务器本地验证通过：`http://127.0.0.1:8000/health` 返回 `{"status":"ok"}`，`http://127.0.0.1:8001/api/auth/ok` 返回 `{"ok":true}`。
- 客户名单页“导入 Excel 客户信息”入口已可见并可正常使用。
- 标准格式 Excel 与自定义格式 Excel 导入流程已可正常使用；导入后客户字段提取、notes 结构化、匹配度空值显示为 `-` 的体验已恢复正常。
- 聊天记录三点菜单、点击外部关闭、左侧内联重命名、直接删除聊天记录均已可正常使用。

### 2026-05-10 历史会话三点菜单：重命名与删除

体验优化：

- `frontend/components/sidebar.tsx`：历史会话列表效仿 ChatGPT，默认只显示标题，鼠标悬停到某条会话时右侧显示三点按钮。
- 三点菜单支持“重命名”和“删除”；重命名会即时更新左侧标题，删除当前会话后回到新聊天页。
- `backend/routers/conversations.py`、`backend/services/conversation_service.py`、`backend/schemas/conversation.py`：新增会话重命名 `PATCH /api/conversations/{conversation_id}` 和删除 `DELETE /api/conversations/{conversation_id}`，并校验当前用户所有权。
- `frontend/lib/api.ts`、`frontend/app/page.tsx`：前端接入重命名/删除接口，失败时回滚本地状态并提示。
...
### 2026-05-10 历史会话时间与上传客户匹配度显示优化

体验问题修复：

- `frontend/components/sidebar.tsx`：左侧历史会话列表不再显示“今天”、具体时间或“刚刚”，只保留会话标题，减少侧边栏视觉噪音。
- `backend/app/services/lead_service.py` / `backend/app/schemas/lead.py`：上传 Excel/CSV 导入的客户如果没有经过 AI 匹配评分，且数据库里的 `match_score` 只是占位 0，则 API 返回 `null`；数据库排序仍按 0 处理，默认降序时排在最后。
- `frontend/types/index.ts`、`frontend/components/customer-list.tsx`、`frontend/components/leads-table-modal.tsx`：客户名单与弹窗里支持空匹配度，显示为 `-`，不再显示误导性的 `0.0`。

验证：

- `python -m compileall backend/app` 通过。
- `npm.cmd run build` 通过，包含前端类型检查。
- `npm.cmd run lint` 未执行完成：当前项目未初始化 Next ESLint 配置，命令会进入交互式配置向导。
...
- 确认当前 CUSTOMER_ACQUISITION_CRAWLER_VERSION 未设置时走 v1；
- 跑一次现有客户开发流程；
- 确认客户名单、导出 Excel、邮件撰写入口正常。
...
### Step 2：增加 feature flag

任务：

- 新增 CUSTOMER_ACQUISITION_CRAWLER_VERSION=v1|v2；
- 默认 v1；
- 在 pipeline_service.py 内读取；
- run_pipeline() 中只在搜索/抓取分支做 v1/v2 分发。

验收：

- 不设置环境变量时走 v1；
- 设置 v2 时进入 v2 分支；
- v2 分支异常时可 fallback v1。

### Step 3：新增 v2 搜索候选评分

任务：

- 新增 _score_search_result_v2；
- 新增 _collect_search_candidates_v2；
- 对 Serper 结果增加内部字段：
  - result_type
  - website_score
  - should_crawl
...
- 能过滤明显 B2B 平台、黄页、新闻、博客、PDF、社媒；
- 能优先保留真实公司官网；
- 不影响 v1 的 _is_company_url 行为。

### Step 4：新增 v2 官网抓取入口

任务：

- 保留 scrape_companies_sync()；
- 新增 scrape_companies_sync_v2()；
- 新增 _scrape_single_company_v2()；
- 每个官网最多抓 3-5 页。
...
- 不生成猜测邮箱；
- 不保留 noreply、example、test、privacy、abuse 等低价值邮箱；
- 主邮箱选择规则稳定。
...
- 从 contact/about/team 页面提取公开联系人；
- 从邮箱附近文本提取姓名和职位；
- 没有明确联系人则留空。
...
- 不编造联系人；
- 不把泛称当联系人；
- contact_name 字段兼容旧 Lead。

### Step 7：社媒链接提取

任务：

- 从官网首页、footer、contact 页面提取官方社媒链接；
- 识别 Facebook、Instagram、YouTube、LinkedIn company、X、TikTok、Pinterest；
- 过滤分享按钮和明显无关链接。
...
- 只有官网无邮箱或邮箱低价值时才触发；
- 优先抓 Facebook、Instagram、YouTube；
- 只抓公开可见内容；
- 提取公开邮箱、电话、地址、简介；
- 找到邮箱后合并到现有 email 字段。
...
- v1 默认流程；
- v2 正常官网有邮箱；
- v2 官网无邮箱但 Facebook 有邮箱；
- v2 社媒是分享按钮，应过滤；
- v2 LinkedIn 需要登录，不应阻断；
- v2 全失败，fallback v1；
- Lead 写入字段兼容；
- 客户名单展示正常；
- 导出 Excel 正常；
...
1. 将 CUSTOMER_ACQUISITION_CRAWLER_VERSION 改回 v1；
2. v1 旧 pipeline 保持完整可用；
3. 不需要数据库回滚；
4. 不影响已有 Lead；
5. 不影响邮件撰写、导出、画像等功能。
...
9. 如何回滚；
10. 当前还存在什么风险。
### 2026-05-11 客户开发爬虫增强 v2 第一版

已完成：
- 新增 `CUSTOMER_ACQUISITION_CRAWLER_VERSION=v1|v2` 配置，默认 `v1`；`.env.example` 已补充默认值。
- `backend/app/services/pipeline_service.py` 新增 v2 搜索候选评分与收集逻辑：保留真实官网，过滤社媒、B2B 平台、黄页/目录、博客新闻、PDF/图片等非公司官网结果。
- `run_pipeline()` 只在客户开发搜索/抓取分支做 v1/v2 分发；v2 搜索无候选或异常时自动 fallback v1。
- `backend/app/utils/scraper.py` 保留 `scrape_companies_sync()`，新增 `scrape_companies_sync_v2()` 和 `_scrape_single_company_v2()`。
- v2 抓取每家公司最多抓首页、contact、about/company、products/services/solutions、team/leadership 等 3-5 页。
- v2 增强公开邮箱提取：正文、`mailto:`、footer、contact/about/team 页面、简单反混淆 `name [at] domain [dot] com`，并过滤 `noreply/example/test/privacy/abuse` 等低价值邮箱。
- v2 新增保守联系人提取：只从公开页面中邮箱附近文本或职务附近文本提取真实出现的人名；没有明确联系人则留空。
- v2 新增官网挂出的社媒链接识别：Facebook、Instagram、YouTube、LinkedIn company、X、TikTok、Pinterest；过滤分享按钮和 LinkedIn 个人页。
- 当官网无高质量邮箱时，v2 会尝试抓取官网挂出的 Facebook / Instagram / YouTube 公开页补充邮箱、电话和简介；不登录、不绕限制、不抓私信/评论、不提交表单。
- v2 抓取异常、全失败或抓完没有可分析正文时自动 fallback v1；失败原因写入日志和任务进度。

验证：
- `python -m compileall backend/app` 通过。
- 函数级验证通过：v2 能保留官网候选，过滤 Facebook 搜索结果、博客/榜单、PDF；能提取 `sales [at] domain [dot] com` 和 `mailto:`；能过滤社媒分享按钮，只保留官网挂出的官方社媒链接。
...
- 未改 SSE 协议。
- 未改 AI 分析提示词主体。
- 未接第三方邮箱验证 API，未做邮箱猜测。

2026-05-11 v2 local smoke follow-up:
- Ran `scrape_companies_sync_v2()` against a temporary local multi-page website with home/contact/about/products pages.
- Verified v2 follows same-site contact/about/products links, scrapes 4 pages, extracts deobfuscated email, `mailto:` email, phone, LinkedIn company link, and a real contact name from title context.
- Fixed two issues found by the smoke test: same-site URL comparison now ignores URL ports, and contact extraction no longer treats nav/company text as a person name.
- Adjusted email ranking so `sales` and `export` beat generic `office/info` when all are same-domain emails.

2026-05-11 v1/v2 same-search timing comparison:
- Test params: `industry=aluminum ceiling`, `country=Canada`, `keywords=[contractor]`, target `num=2`; compared search + scrape only, no DB writes and no Replicate analysis.
- Initial v2 run found more contact data but was too slow because it scanned all 17 generated queries; optimized `_collect_search_candidates_v2()` to stop after building a scored candidate pool.
- Also filtered third-party technical emails from page scripts, including Sentry/Wix addresses and long hex local parts.
- After optimization: v1 total `69.30s` (`search=2.31s`, `scrape=67.00s`, `queries=1`, `emails=2/6`, `phones=3/6`, `scraped_ok=4/6`).
- After optimization: v2 total `72.72s` (`search=6.37s`, `scrape=66.35s`, `queries=2`, `emails=4/6`, `phones=4/6`, `scraped_ok=4/6`).
- Result: v2 is about `3.42s` slower on this sample, but doubled email coverage and improved phone coverage. Contact-name coverage remained `0/6` for both because tested sites did not expose clear person names in title context.

2026-05-11 V2.1 small enhancement:
- Added V2-only query expansion path. V1 `_generate_queries()` remains unchanged. V2.1 uses the model to generate 5-8 precise search queries and falls back to the original `_generate_queries()` when expansion fails or no model token is available.
- Added backend-only compatibility for `customer_types` inside pipeline params and `StartPipelineRequest`; `/api/tasks/start` now passes the optional value through for customer acquisition only. No frontend UI changes were made; missing values default to `["manufacturer", "supplier", "company"]`.
- Upgraded V2 search scoring from one score to sub-scores: `_official_score`, `_industry_score`, `_country_score`, `_role_score`, `_contact_likelihood_score`, plus `_website_score`, `_select_reason`, `_reject_reason`, `_role_evidence`, and `_inferred_role_type`.
- Added light candidate diversity selection: V2.1 first tries to keep high-quality candidates for requested customer roles, then fills remaining slots by score without retaining low-quality websites.
- Added V2 candidate context to the existing AI analysis input as reference fields only: `select_reason`, `inferred_role_type`, `role_evidence`, and scoring details. Final Lead columns remain unchanged.
- Function-level tests passed for customer type normalization, query-expansion fallback, scoring subfields, social/PDF rejection, and candidate diversity.
- Real lightweight V2.1 search test passed for `aluminum ceiling + Canada + manufacturer/supplier`: model expanded 8 queries in `3.51s`; Serper candidate collection took `28.16s`; selected 6 candidates with select reasons and role evidence.
- Validation: `python -m compileall backend/app` passed; `git diff --check` passed.

2026-05-11 V2.1 next TODO:
- ~~Frontend customer type multi-select UI~~ ✅ Done
- ~~Frontend payload wiring~~ ✅ Done
- ~~UI defaults~~ ✅ Backend defaults `["manufacturer", "supplier", "company"]` when not selected
- ~~Keep V1/V2 behavior~~ ✅ No longer needed — V2 is now default (see below)
- ~~Regression checks~~ ✅ `npm.cmd run build` passed; `python -m compileall` passed; `git diff --check` passed

2026-05-11 Frontend customer type multi-select UI:
- Added `CustomerType` type, `CUSTOMER_TYPE_OPTIONS` const array, and `customerTypes` field to `ConfirmParamsData` in `frontend/types/index.ts`.
- Added toggle-button multi-select UI in `confirm-params-card.tsx`: 8 customer type options (manufacturer/supplier/distributor/contractor/installer/brand/competitor/buyer), displayed below keywords, labeled "客户类型（可选）". Selected buttons turn brand-blue; unselected remain gray.
- Wired `customerTypes` through `onConfirm` → `handleConfirmParams` → `startConfirmedPipeline()` as optional `customerTypes: string[]` in the POST body. Updated prop types in `message-bubble.tsx`, `message-list.tsx`, `chat-area.tsx`, `page.tsx`, `api.ts`.
- When user selects nothing, `customerTypes` is omitted from the request (backend defaults to `["manufacturer", "supplier", "company"]`). When selected, array is sent as `customerTypes` in JSON body, deserialized by backend `CamelModel` as `customer_types`.
- No changes to DB schema, Lead model, SSE protocol, email-craft, email-blast, or company-profile pipelines.

2026-05-11 Switch default crawler to V2:
- Changed `customer_acquisition_crawler_version` default from `v1` to `v2` in `backend/app/config.py`, `backend/.env`, and `.env.example`.
- Decision based on full pipeline benchmark (`solar panel mounting system + Australia + distributor/installer, num=3`):
  - V2.1 AI analysis completeness: 3/3 leads had ai_summary + business_match + outreach (V1: 2/3, one lead missing business_match and outreach).
  - V2.1 avg confidence score: 68.3 vs V1's 48.3 (+20pt).
  - V2.1 avg business_match length: 88ch vs V1's 61ch; avg outreach length: 90ch vs 65ch.
  - V2.1 email coverage: 3/3 vs V1's 2/3.
  - V2.1 role-targeted selection: found actual distributors/installers (Sol Distribution, Solar Install Co) instead of generic manufacturers (Kuga, Nova).
  - V2.1 tradeoff: ~33s slower total (124s vs 91s), mainly due to AI query expansion adding 2s + longer search phase (24s vs 2s). Acceptable given quality improvement.
- Rollback: set `CUSTOMER_ACQUISITION_CRAWLER_VERSION=v1` in `.env` to revert.

2026-05-11 Deploy to server (blocked):
- Commits `37a907b`, `8611a3c` pushed to `origin/main`. Code pull succeeded on server.
- **卡点**: 服务器 `127.0.0.1:8000` 端口被一个非 PM2 管理的 uvicorn 残留进程占用，PM2 `waimao-api` 反复重启失败（`Errno 98: address already in use`）。
  - 已尝试：`lsof -ti:8000 | xargs kill -9`、`pkill -9 -f uvicorn`、`pkill -9 python3`、`kill -9 <pid>`，每次杀掉后端口仍被新进程立即占回。
  - 怀疑原因：可能有 systemd service、cron job、或 supervisor 在自动拉起 uvicorn 进程，需要排查 `systemctl list-units --type=service | grep -i waimao`、`crontab -l`、`cat /etc/supervisor/conf.d/*.conf`。
  - 服务器信息：腾讯云香港轻量 `VM-0-13-ubuntu`，项目路径 `/var/www/hyyskill/backend`，venv 在 `/var/www/hyyskill/backend/.venv/`。
  - 部署指南在 `doc/deploy.md`（日常更新只需 `cd /var/www/hyyskill && git pull origin main && pm2 restart waimao-api`）。
  - 前端 Vercel 已自动部署，不受影响。后端当前仍运行旧代码（残留 uvicorn 进程），但 PM2 无法接管。
- No new Python dependencies, no new DB migrations needed.

2026-05-11 Deploy to server (resolved follow-up):
- Root cause: the server had two PM2 daemons. `root` PM2 (`/root/.pm2`) was managing an old `waimao-api` process and occupying `127.0.0.1:8000`; `ubuntu` PM2 (`/home/ubuntu/.pm2`) was also managing `waimao-api` and `waimao-auth`, so ubuntu `waimao-api` kept failing with `Errno 98: address already in use`.
- Confirmed the "resurrecting" uvicorn process was parented by the root PM2 daemon, not by a separate systemd, supervisor, or cron job.
- Fix applied on server:
  - Deleted the root-owned process with `pm2 delete waimao-api`.
  - Saved the root PM2 empty process list state.
  - Let ubuntu PM2 take over `waimao-api` on port 8000.
  - Restarted ubuntu PM2 `waimao-api` from `/var/www/hyyskill/backend/ecosystem.config.cjs`.
  - Ran `pm2 save` under ubuntu so the correct process list persists.
- Deployment update completed:
  - Server pulled `origin/main` from `8611a3c` to `9803f72`.
  - `waimao-api` restarted as ubuntu user and is online.
  - `waimao-auth` remains online as ubuntu user.
- Verification passed:
  - `curl -s http://127.0.0.1:8000/docs | head -3` returned HTML.
  - `curl -s http://127.0.0.1:8000/health` returned `{"status":"ok"}`.
  - `sudo -iu ubuntu pm2 status` shows both `waimao-api` and `waimao-auth` online.
- Current production process ownership rule: use ubuntu PM2 only. Avoid starting or restarting app PM2 processes as root.
- Remaining small cleanup: server worktree has untracked `auth-service/package-lock.json`; it did not block deploy. Decide later whether to commit it or remove it after confirming it is unnecessary.

2026-05-11 Current completed progress:
- Production API/Auth/DB are running on Tencent Cloud Hong Kong server `43.128.3.59`.
- Backend project path: `/var/www/hyyskill`; FastAPI backend path: `/var/www/hyyskill/backend`; Python venv: `/var/www/hyyskill/backend/.venv`.
- PM2 production apps:
  - `waimao-api` -> FastAPI/Uvicorn on `127.0.0.1:8000`.
  - `waimao-auth` -> Better Auth Node service on `127.0.0.1:8001`.
- Daily deploy flow remains: `cd /var/www/hyyskill && git pull origin main && sudo -iu ubuntu pm2 restart waimao-api`.
- Customer acquisition pipeline is now using the V2.1-enhanced path via `CUSTOMER_ACQUISITION_CRAWLER_VERSION=v2`.
- V2.1 means: AI query expansion, role-aware candidate scoring, candidate diversity selection, V2 scraping, and automatic fallback to V1 on V2 search/scrape failure.
- Code config still uses value `v2`; there is no separate `v2.1` env value.
- `email-craft`, `email-blast`, and `company-profile` pipelines were not changed by V2.1 and remain on their stable paths.

### 2026-05-11 意图识别优化 + 开发信用户额外需求

本轮优化 intent_router 的 3 个误分类问题，并新增开发信撰写时的"用户额外要求"功能。

**意图识别改进（intent_router.py）**：
- 新增 `view_profile` 动作：用户说"我的公司信息是什么"、"看一下公司资料"等 → 查看已有公司画像，返回带"查看公司资料"按钮的卡片，不触发 pipeline。
- 新增第一人称公司表述检测：用户说"我们做过XX项目"、"我们公司有XX产品"等 → `company_profile` (update)，不再误判为 `customer_acquisition`。
- 新增历史记录查找模式："帮我找我们做过的项目"、"找天花客户我们做过白云机场"等（找+我们做过/合作过）→ `view_profile`（查历史），而非找新客户或修改画像。
- 新增空修改请求检测：用户说"公司信息修改"但没说改什么 → 返回 chat 提示用户说明要修改什么，不空跑 pipeline。
- 搜索意图与第一人称的优先级规则：有明确"找新客户"意图时搜索优先，纯描述公司信息时画像优先。
- 邮件内容注入改为 email_craft：用户说"开发信里要提白云机场项目" → `email_craft`（带 `user_requirements`），而非 `company_profile`。

**开发信用户额外需求功能（全链路）**：
- 意图识别提取 `user_requirements` 参数（LLM prompt + regex fallback）。
- 确认卡片（email-craft-confirm-card）新增"补充要求"textarea，预填 user_requirements，用户可编辑。
- `user_requirements` 通过 confirm card → chat_service → StartPipelineRequest → task params → email_craft_pipeline 全链路透传。
- Pipeline 的 `build_user_prompt()` 在客户信息和我司信息之间插入 `=== 用户额外要求 ===` 段。
- 从客户名单页面点"生成开发信"时，通过 `pendingEmailRequirementsRef` 跨组件传递 requirements。
- 无额外要求时行为与改动前完全一致（该段不出现）。

**改动文件**：
- `backend/app/services/intent_router.py` — 新增 view_profile 动作、第一人称检测、历史查找、空修改检测、user_requirements 提取
- `backend/app/services/chat_service.py` — view_profile handler、needs_clarification handler、user_requirements 透传
- `backend/app/services/email_craft_pipeline_service.py` — build_user_prompt 加用户额外要求段
- `backend/app/schemas/chat.py` — StartPipelineRequest 加 user_requirements 字段
- `backend/app/routers/tasks.py` — 传 user_requirements 到 pipeline
- `frontend/types/index.ts` — EmailCraftConfirmData 加 userRequirements
- `frontend/lib/api.ts` — startConfirmedPipeline params 加 userRequirements
- `frontend/components/email-craft-confirm-card.tsx` — 新增补充要求 textarea
- `frontend/app/page.tsx` — pendingEmailRequirementsRef 跨页面传递
