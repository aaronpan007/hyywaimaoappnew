...
- Frontend customer type multi-select UI: add customer type selection to the customer-acquisition confirmation card only. Do not redesign the chat UI. Suggested values: `manufacturer`, `supplier`, `distributor`, `contractor`, `installer`, `brand`, `competitor`, `buyer`.
- Frontend payload wiring: pass selected values as `customerTypes` through `startConfirmedPipeline()` to `/api/tasks/start`. Backend already accepts camelCase via `StartPipelineRequest.customer_types`.
- UI defaults: if user does not touch the selector, default to `["manufacturer", "supplier", "company"]` backend-side; frontend may display manufacturer/supplier/company as the default intent if convenient.
- Keep V1/V2 behavior: customer type selection should only affect V2/V2.1 internals; V1 remains default unless `CUSTOMER_ACQUISITION_CRAWLER_VERSION=v2`.
- Add/extend frontend types only as needed. Do not change Lead fields, DB schema, SSE protocol, email-craft, email-blast, or company-profile.
- Regression checks after UI wiring: existing customer-acquisition confirm card still starts tasks without customerTypes; starting with customerTypes sends the array in request body; `npm.cmd run build` passes; backend `python -m compileall backend/app` still passes.
- Optional next backend tuning after UI: run three more same-search comparisons with real Serper only, no DB writes and no Replicate analysis: `aluminum ceiling + USA + contractor/distributor`, `building materials + Australia + supplier`, and an old request without customer_types.
